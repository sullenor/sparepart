 ...
 * @param {Core}   parent
 */
function ElemMod(directory, parent) {
  Core.call(this, directory, parent);

  if (!parent) {
    var elempath = path.dirname(directory);
    this._block = path.basename(path.dirname(elempath));
    this._elem = path.basename(elempath);
  }
...
inherits(ElemMod, Core);

var proto = ElemMod.prototype;
...
proto._determine = function (filepath, filename) {
  ...
  if (!/^[\-a-z0-9]+__[\-a-z0-9]+_[\-a-z0-9]+_[\-a-z0-9]+$/i.test(basename)) {
...
proto._resolveFilename = function (suffix) {
  return suffix.replace('?', format('%s__%s_%s',
    this._block,
    this.name));
};
